...
if [ -d "$KUBEADDONS_ENTERPRISE_ABS_PATH" ]
then
  pushd $KUBEADDONS_ENTERPRISE_ABS_PATH
...
do
    printf "addon directory detected: %s directory\n" "$i"
    if [ ! -d "./tests/$i" ]
    then
...
for i in $(find "${KUBEADDONS_ENTERPRISE_PATH}"/addons -mindepth 2 -type d)
do
    KUBEADDONS_ENTERPRISE_PATH=${KUBEADDONS_ENTERPRISE_ABS_PATH} kubectl kuttl test tests/"$i" | tee /dev/fd/2 | go-junit-report -set-exit-code > dist/addons_test_report.xml
done
